...
{
  json_writer_time(&s->writer, value);
}
...
MMAPString * mime_serializer_serialize_message(struct mailmime * mime)
{
  ...
  s.writer.out = mmap_string_new("");
  assert(s.writer.out != NULL);
