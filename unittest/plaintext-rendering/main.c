...
static char * render_message_html(struct mailmime * mime)
{
  struct render_context context;
  char * header;
  char * body;
    ...
    const char * input_root, const char * output_root)
{
  ...
  }
  {
    ...
    current_default_charset = detected != NULL ? detected : "utf-8";
  }
  ...
  mailmime_free(mime);
  free(expected_path);
  if (current_default_charset != NULL &&
      strcmp(current_default_charset, "utf-8") != 0)
    free((char *) current_default_charset);
  current_default_charset = "utf-8";
  free(data);
}
