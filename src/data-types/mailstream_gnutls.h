#ifndef MAILSTREAM_GNUTLS_H
#define MAILSTREAM_GNUTLS_H

#include <libetpan/mailstream_ssl.h>

#ifdef HAVE_GNUTLS
mailstream_low_driver * mailstream_gnutls_low_driver(void);
mailstream_low * mailstream_gnutls_open_low(int fd, int starttls,
    time_t timeout,
    void (* callback)(struct mailstream_ssl_context *, void *), void * data);
ssize_t mailstream_gnutls_ssl_get_certificate(mailstream * stream,
    unsigned char ** der);
int mailstream_gnutls_ssl_set_client_certicate(
    struct mailstream_ssl_context * context, char * filename);
int mailstream_gnutls_ssl_set_client_certificate_data(
    struct mailstream_ssl_context * context, unsigned char * der,
    size_t length);
int mailstream_gnutls_ssl_set_client_private_key_data(
    struct mailstream_ssl_context * context, unsigned char * der,
    size_t length);
int mailstream_gnutls_ssl_set_server_certicate(
    struct mailstream_ssl_context * context, char * ca_file, char * ca_path);
int mailstream_gnutls_ssl_set_server_name(
    struct mailstream_ssl_context * context, const char * hostname);
int mailstream_gnutls_ssl_get_fd(struct mailstream_ssl_context * context);
void mailstream_gnutls_ssl_init_not_required(void);
void mailstream_gnutls_ssl_init_lock(void);
void mailstream_gnutls_ssl_uninit_lock(void);
#endif

#endif
